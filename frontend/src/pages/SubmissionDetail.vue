...
<template>
  <div v-if="loading" class="max-w-4xl mx-auto p-6 flex items-center justify-center">
    <div class="flex flex-col items-center gap-4">
      <div class="w-8 h-8 border-4 border-zinc-700 border-t-zinc-400 animate-spin"></div>
      <div class="text-zinc-400">加载中...</div>
    </div>
  </div>

  <div v-else-if="error" class="max-w-4xl mx-auto p-6">
    <div class="bg-red-900/30 border border-red-800 text-red-400 p-4">
      {{ error }}
    </div>
  ...
  <div v-else-if="sub" class="space-y-5">
    <div class="flex flex-wrap items-center gap-3">
      ...
      <!-- 评测中指示器 -->
      <div v-if="judging" class="flex items-center gap-2 text-sm text-zinc-400">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
      <div class="card p-3">
        <div class="text-zinc-400 text-xs">题目</div>
      ...
      <div class="card p-3"><div class="text-zinc-400 text-xs">内存</div><div class="font-medium mt-0.5">{{ sub.memory_kb ? (sub.memory_kb / 1024).toFixed(1) + ' MB' : '-' }}</div></div>
    </div>
    ...
    <div v-if="cases.length || judging" class="card overflow-hidden">
      <div class="px-4 py-2.5 border-b border-zinc-700 bg-zinc-800 text-sm font-semibold">测试点信息</div>
      <div class="p-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          <div 
            v-for="(c, i) in (judging ? [] : cases)" 
            :key="i"
            @click="selectedCase = selectedCase === i ? null : i"
            :class="[
              'relative p-4 cursor-pointer border-2',
              selectedCase === i ? 'border-zinc-500' : 'border-zinc-700',
              getTestCaseInfo(c, i).config.color
            ]"
          >
            <div class="text-center">
              <div class="text-sm font-bold">#{{ i + 1 }}</div>
              <div class="text-lg font-bold mt-2">{{ getTestCaseInfo(c, i).config.label }}</div>
              <div v-if="c.passed && c.timeMs" class="text-xs mt-2 opacity-90">
                {{ c.timeMs }}ms/{{ c.memoryKb ? (c.memoryKb/1024).toFixed(0) + 'KB' : '0KB' }}
              </div>
              <div v-else-if="c.passed" class="text-xs mt-2 opacity-90">
                {{ c.timeMs }}ms/0KB
              </div>
            </div>
          </div>
          ...
          <div 
            ...
            v-for="i in 10"
            :key="'loading-' + i"
            class="relative p-4 border-2 border-zinc-700 bg-zinc-800"
          >
            <div class="text-center">
              <div class="text-sm font-bold text-zinc-400">#{{ i }}</div>
              <div class="flex items-center justify-center mt-2">
                <div class="w-6 h-6 border-2 border-zinc-600 border-t-zinc-400 animate-spin"></div>
              </div>
              <div class="text-xs mt-2 text-zinc-500">评测中</div>
            </div>
          </div>
        </div>
        
        <!-- 测试点详情 -->
        <div v-if="selectedCase !== null && !judging" class="mt-4 p-4 bg-zinc-800 border border-zinc-700">
          ...
          <div v-if="!cases[selectedCase].passed" class="grid sm:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <pre class="!text-xs !p-2 !bg-zinc-900 border border-zinc-700">{{ cases[selectedCase].expected }}</pre>
            </div>
            <div>
              <div class="text-zinc-400 mb-1">你的输出</div>
              <pre class="!text-xs !p-2 !bg-rose-950 border border-rose-900">{{ cases[selectedCase].actual }}</pre>
            </div>
          </div>
    ...
    <div class="card overflow-hidden">
      <div class="flex items-center justify-between px-4 py-2.5 border-b border-zinc-700 bg-zinc-800 text-sm">
        <span class="font-semibold">提交代码</span>
        <span class="text-xs text-zinc-400">{{ new Date(sub.created_at).toLocaleString() }}</span>
      </div>
